...
def generate_spdx_license_list(dest = "lib/rubygems/util/licenses.rb")
  ...
  updates = [
    ...
    (base+"#{u}.json").open do |f|
      next Time.parse(f.meta["last-modified"]), JSON.parse(f.read)[u].map {|o| o[i] }
    end
  end

  mtime, = updates.max_by {|t,| t}
...
RUBY
  ...
  return if content == File.read(dest) rescue nil
